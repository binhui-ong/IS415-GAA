...
However, most of the modelling packages in R is expecting a matrix look, with rows representing **origins** (i.e. also know as **from** field) and the columns representing **destination** (i.e. also known as **to** field.)
...
#| eval: false
#| echo: false
write_rds(distmat_km, "data/rds/distmat_km.rds")
```

```{r}
#| echo: false
distmat_km <- read_rds("data/rds/distmat_km.rds")
```

# 6 Modelling and Visualizing Hansen's Accessibility using Hansen Method
...
#| eval: false
#| echo: false
write_rds(acc_Hansen, "data/rds/acc_Hansen.rds")
```

```{r}
#| echo: false
acc_Hansen <- read_rds("data/rds/acc_Hansen.rds")
```
...
#| eval: false
#| echo: false
write_rds(hexagon_Hansen, "data/rds/hexagon_Hansen.rds")
```

```{r}
#| echo: false
hexagon_Hansen <- read_rds("data/rds/hexagon_Hansen.rds")
```
...
## 6.2 Visualizing Hansen's accessibility
...
![](images/pic6.3%20.png)

# 7 Modelling and Visualizing Accessibility using KD2SFCA method
...
## 7.2 Visualizing KD2SFCA's accessibility
...
# 8 Modelling and Visualizing Accessibility using Spatial Accessibility Measure (SAM)
...
## 8.2 Visualizing SAM's accessibility
