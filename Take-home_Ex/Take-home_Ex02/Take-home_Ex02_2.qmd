...
-   mutate() to create a new column consisting of local_moran's I statistics
...
-   unnest() to expand the list-coumn containing the resulting data frame of local moran's I statistics into columns
...
![](images/local morans i of total cases.png)
...
![](images/p-value of local morans i.png)
...
Putting the local moran's I value and p-value choropleth maps side by side allows to interpret out results more easily, tracing which villages are significant outliers or significant clusters on the map. To do so, we can use tamp_arrange() to put the maps together, as demonstrated in the code chunk below.
...
With the Gi\* measures, we can evaluate each location for a trend using the Mann-Kendall test. For instance, we can evaluate the trend for location Andong Vil. , D06 by using the code chunk below.
...
Then, we plot the result using ggplot2 functions.
...
Alternatively, we can create an interactive plot using ggplotly() of plotly package.
